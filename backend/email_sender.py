...
def send_email_with_attachment(to, bodyUuid, attachmentUuid, attachmentFilePath = '', body='', subject="No Subject", encryption_flag="0"):
    ...
    print(sender_email, receiver_email, password, "ll")
    ...
    if encryption == "1":
        # OTP Encryption
        print(bodyUuid)
        print(attachmentUuid)
        bodyUuid = '(' + ",".join(bodyUuid) + ')'
        print(type(bodyUuid))
        ...
        message.add_header("X-Attachment-UUID", '[]')#email_details['attachmentUuid'])
    elif encryption == "2":
        # AES Encryption
        bodyUuid = '(' + ",".join(bodyUuid) + ')'
        message.add_header("X-Body-Encryption", "AES")
        message.add_header("X-Body-UUID", bodyUuid)
